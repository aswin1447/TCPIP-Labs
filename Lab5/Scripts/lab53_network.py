...
def run():
    ...
    link_r1sw2.intf1.config( bw=2, enable_red=True, enable_ecn=True)
    link_r2sw3.intf1.config( bw=2, enable_red=True, enable_ecn=True)
    ...
    h2.cmd('ip route add default via 10.0.2.20') # OK
    h3.cmd('ip route add default via 10.0.3.20') # OK

    r2.cmd('route add -net 10.0.1.1/24 gw 10.0.10.10')

    r1.cmd('route add -net 10.0.3.3/24 gw 10.0.10.20')
    r1.cmd('route add -net 10.0.2.2/24 gw 10.0.10.20')
    ...
    r1.cmd('ip route add 10.0.1.1/24 via 10.0.1.10')
    r1.cmd('ip route add default via 10.0.10.20')
    r2.cmd('ip route add default via 10.0.10.10')
    r2.cmd('ip route add 10.0.2.2/24 via 10.0.2.20')
    r2.cmd('ip route add 10.0.3.3/24 via 10.0.3.20')

    h1.cmd('tc qdisc add dev h1-eth0 root netem delay 150ms')
    h2.cmd('tc qdisc add dev h2-eth0 root netem delay 150ms')
    h3.cmd('tc qdisc add dev h3-eth0 root netem delay 150ms')
